...
class Department 
{
    ...
    public function create($data)
    {
        ...
        if (isset($response->result)) {
            $this->defineMensagem('success', "Departamento criado com sucesso!");
            $this->router->redirect("/department");
            return false;
        }

        $this->defineMensagem('error', "Não foi possivel cadastrar o departamento!");
        $this->router->redirect("/department");
    }
    ...
    public function index($data)
    {
        ...
        curl_setopt_array($this->curl, array(
            CURLOPT_URL => "http://api.ideris.com.br/Departamento/?limit=10",
        ...
        $paginator = new Paginator(URL_BASE . "department/", '', ["Primeira Página", "Primeira"], ["Última Página", "Última"]);
        $paginator->pager($response->paging->total, 10, $page, 2);
        
        $this->curl = curl_init();
        
        curl_setopt_array($this->curl, array(
            CURLOPT_URL => "http://api.ideris.com.br/Departamento/?offset={$paginator->offset()}&limit={$paginator->limit()}",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        curl_close($this->curl);
        echo $this->view->render('departamento', [
            'department' => $response,
            'paginator' => $paginator
            ]
        );
    ...
    public function query($data)
    {
        ...
        $url = "http://api.ideris.com.br/Departamento?limit=10";
        ...
        $paginator = new Paginator(URL_BASE . "department/query/", '', ["Primeira Página", "Primeira"], ["Última Página", "Última"]);
