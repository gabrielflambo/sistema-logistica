...
class Category 
{
    ...
    public function create($data)
    {
        ...
        if (isset($response->result)) {
            $this->defineMensagem('success', "Categoria criada com sucesso!");
            $this->router->redirect("/category");
            return false;
        }

        $this->defineMensagem('error', "Não foi possivel cadastrar a categoria!");
        $this->router->redirect("/category");
    }
    ...
    public function index($data)
    {
        ...
        curl_setopt_array($this->curl, array(
            CURLOPT_URL => "http://api.ideris.com.br/Categoria/?limit=10",
        ...
        $paginator = new Paginator(URL_BASE . "category/", '', ["Primeira Página", "Primeira"], ["Última Página", "Última"]);
        $paginator->pager($response->paging->total, 10, $page, 2);
        
        $this->curl = curl_init();
        
        curl_setopt_array($this->curl, array(
            CURLOPT_URL => "http://api.ideris.com.br/Categoria/?offset={$paginator->offset()}&limit={$paginator->limit()}",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public function query($data)
    {
        ...
        $url = "http://api.ideris.com.br/Categoria?limit=10";
        ...
        $paginator = new Paginator(URL_BASE . "category/query/", '', ["Primeira Página", "Primeira"], ["Última Página", "Última"]);
