...
class Press
{
    ...
    public function index($data)
    {
        ...
        curl_setopt_array($this->curl, array(
            CURLOPT_URL => "http://api.ideris.com.br/ListaPedido?limit=10&Status=".htmlentities(urlencode('Impressão')),
        ...
        $paginator = new Paginator(URL_BASE . "press/", '', ["Primeira Página", "Primeira"], ["Última Página", "Última"]);
        $paginator->pager($response->paging->total, 10, $page, 2);
        
        $this->curl = curl_init();
        
        curl_setopt_array($this->curl, array(
            CURLOPT_URL => "http://api.ideris.com.br/ListaPedido?offset={$paginator->offset()}&limit={$paginator->limit()}&Status=".htmlentities(urlencode('Impressão')),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public function search($data)
    {
        ...
        $url = "http://api.ideris.com.br/ListaPedido?limit=10&Status=".htmlentities(urlencode('Impressão'));
        ...
        $paginator = new Paginator(URL_BASE . "press/search/", '', ["Primeira Página", "Primeira"], ["Última Página", "Última"]);
        $paginator->pager($response->paging->total, 10, $page, 2);
        
        $url = "http://api.ideris.com.br/ListaPedido?offset={$paginator->offset()}&limit={$paginator->limit()}&Status=".htmlentities(urlencode('Impressão'));
